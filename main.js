...
async function new_with_state(id, state) {
    ...
    if (new_priority == null || new_priority.val === 0) {
        new_priority = ""
    }
    ;
    if (new_date == null || new_date.val === 0) {
        new_date = ""
    }
    ;
    if (new_label == null || new_label.val === 0) {
        new_label = ""
    }
    ;
    if (new_project == null || new_project.val === 0) {
        new_project = ""
    }
    ;
    //Debug ausgabe:
...
async function getRAW() {
    ...
    if (adapter.config.project_collaborators === true) {
        ...
        await adapter.setStateAsync("RAW.Project_Collaborators", {
            val: JSON.stringify(all_collaborators_objects),
            ack: true
        });
    }
...
async function tasktotask() {
    ...
    for (i = 0; i < json.length; i++) {
        ...
        if (adapter.config.json_objects === true) {
            var prio_neu = 0;
            await helper.reorder_prio(json[i].priority).then(data => {
                prio_neu = data
            });
            await json_verarbeitung.table_json(adapter, json[i], prio_neu, all_project_objekts, all_label_objekts, all_sections_objects, all_collaborators_objects).then(data => {
                json_neu_parse.push(data);
            });
...
async function tasktoproject(project) {
    ...
    for (j = 0; j < project.projects_id.length; j++) {
        ...
        var HTMLstring = "";
        if (adapter.config.html_objects == true) {
            await html_verarbeitung.heading_html(adapter).then(data => {
        ...
        for (i = 0; i < json.length; i++) {
            ...
            if (Liste === project.projects_id[j]) {
                ...
                if (adapter.config.html_objects == true) {
                    ...
                    await html_verarbeitung.table_html(adapter, json[i], prio_neu, all_project_objekts, all_label_objekts, all_sections_objects, all_collaborators_objects).then(data => {
                        HTMLstring = HTMLstring + data
                    });
        ...
        if (adapter.config.html_objects == true) {
            ...
            if (json_task === "[]") {
                if (adapter.config.html_visable == false) {
                    ...
                    await html_verarbeitung.table_html_empty(adapter).then(data => {
                        HTMLstring = HTMLstring + data
                    });
                }
            }

            adapter.setState('HTML.Projects-HTML.' + project.projects_names[j], {
                val: '<style>' + css + css2 + '</style>' + '<script>' + 'function myFunction(id) {servConn.setState("todoist2.0.Control.Close.ID", id)}' + '</script>' + '<table id="task_table">' + HTMLstring + '</table>',
                ack: true
            });
        }
        ...
        if (adapter.config.json_objects == true) {
            if (json_task === "[]") {
                await json_verarbeitung.table_json_empty(adapter).then(data => {
                    json_task_parse.push(data);
                });
            }
...
async function tasktolabels(labels) {
    ...
    for (j = 0; j < labels.labels_id.length; j++) {
        ...
        if (adapter.config.html_objects == true) {
            ...
            adapter.setState('HTML.Labels-HTML.' + labels.labes_names[j], {
                val: '<style>' + css + css2 + '</style>' + '<script>' + 'function myFunction(id) {servConn.setState("todoist2.0.Control.Close.ID", id)}' + '</script>' + '<table id="task_table">' + HTMLstring + '</table>',
                ack: true
            });
        }
    ...
    return new Promise(async function (resolve, reject) {
        ...
        if (filter_json.length == 0) {
            if (adapter.config.html_objects == true) {
                ...
                adapter.setState('HTML.Filter-HTML.' + filter_name, {
                    val: '<style>' + css + css2 + '</style>' + '<script>' + 'function myFunction(id) {servConn.setState("todoist2.0.Control.Close.ID", id)}' + '</script>' + '<table id="task_table">' + HTMLstring + '</table>',
                    ack: true
                });
            }
        ...
        for (j = 0; j < filter_json.length; j++) {
            ...
            //Fehler in der Priorität anpassen - es kommen die Falschen zahlen umgedreht:
            var prio_neu = 0;
            await helper.reorder_prio(json[j].priority).then(data => {
                prio_neu = data
            });

            await html_verarbeitung.table_html(adapter, json[j], prio_neu, all_project_objekts, all_label_objekts, all_sections_objects, all_collaborators_objects).then(data => {
                HTMLstring = HTMLstring + data
            });
            ...
            //JSON
            await json_verarbeitung.table_json(adapter, json[j], prio_neu, all_project_objekts, all_label_objekts, all_sections_objects, all_collaborators_objects).then(data => {
                json_task_parse.push(data);
            });
